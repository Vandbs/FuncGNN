...
class AIG_data(InMemoryDataset):
    ...
    def process(self):
        ...
        with open(self.raw_paths[0], 'r') as f:
            ...
            # Statistics of edge types in the current AIG graph
            total_edges = len(edge_weight)
            and_edges = sum(1 for w in edge_weight if w == 1)
            not_edges = sum(1 for w in edge_weight if w == -1)
            if total_edges > 0:
                print("Edge type statistics for current AIG graph:")
                print(f"Total edges: {total_edges}")
                print(f"AND edges (weight 1): {and_edges} Ratio: {and_edges / total_edges * 100:.2f}%")
                print(f"NOT edges (weight -1): {not_edges} Ratio: {not_edges / total_edges * 100:.2f}%")
            else:
                print("No edge information in current AIG graph.")

            # Append statistics to a CSV file and save to the specified folder
            import csv
            csv_file = r"E:\Paper_project\2_EDA\PolarGate-main\edge_stats.csv"
            os.makedirs(r"E:\Paper_project\2_EDA\PolarGate-main", exist_ok=True)
            file_exists = os.path.exists(csv_file)
            with open(csv_file, "a", newline="") as csvfile:
                writer = csv.writer(csvfile)
                if not file_exists:
                    writer.writerow(["GraphName", "TotalEdges", "ANDEdges", "ANDRatio(%)", "NOTEdges", "NOTRatio(%)"])
                if total_edges > 0:
                    and_ratio = and_edges / total_edges * 100
                    not_ratio = not_edges / total_edges * 100
                else:
                    and_ratio = not_ratio = 0
                writer.writerow([self.name, total_edges, and_edges, f"{and_ratio:.2f}", not_edges, f"{not_ratio:.2f}"])

            edge_index = [[edge[0], edge[1]] for edge in data]
...
def sample_per_class(random_state: np.random.RandomState, labels: List[int], num_examples_per_class: Union[int, float],
                     forbidden_indices: Optional[List[int]] = None, force_indices: Optional[List[int]] = None) -> List[
    int]:
...
def get_train_val_test_seed_split(random_state: np.random.RandomState,
                                  ...
                                  train_size: Union[int, float] = None, val_size: Union[int, float] = None,
                                  test_size: Union[int, float] = None, seed_size: Union[int, float] = None) -> Tuple[
...
def load_aig_data(dataset: str = 'deepgate', root: str = './tmp_data/',
                  ...
                  train_size_per_class: Union[int, float] = None, val_size_per_class: Union[int, float] = None,
                  test_size_per_class: Union[int, float] = None, seed_size_per_class: Union[int, float] = None,
                  seed: List[int] = [], data_split: int = 10) -> List[SignedData]:
    raw_dataset = AIG_data(name=dataset, root=root, transform=transform, pre_transform=pre_transform)
    signed_dataset = []
    for graph in raw_dataset:  # Iterate over all AIG graphs
        total_edges = graph.edge_weight.numel()
        if total_edges > 0:
            and_edges = (graph.edge_weight == 1).sum().item()
            rate = and_edges / total_edges
        else:
            rate = 0.0

        signed_graph = SignedData(edge_index=graph.edge_index, edge_weight=graph.edge_weight, init_data=graph)
        rate = rate/(1-rate)
        signed_graph.rate = float(rate)

        if train_size is not None or train_size_per_class is not None:
            signed_graph.node_split(train_size=train_size, val_size=val_size,
                                    test_size=test_size, seed_size=seed_size, train_size_per_class=train_size_per_class,
                                    val_size_per_class=val_size_per_class, test_size_per_class=test_size_per_class,
                                    seed_size_per_class=seed_size_per_class, seed=seed, data_split=data_split)
        signed_dataset.append(signed_graph)
