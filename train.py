...
# os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"  # for old devices
import argparse
import os
import random
import sys
...
def parameter_parser():
    ...
    parser.add_argument('--model', type=str, default='FuncGNN')
...
def load_data_signed_parallel(args, graph_dirs, pi_edges_dict, tt_pair_dict):
    ...
    def process_data(data_dir):
        ...
        graphs = load_aig_data(dataset=args.dataset, root=data_dir, train_size=0.8, val_size=0.1,
                                test_size=0.1, data_split=1)
        for data in graphs:
            data = data.to(args.device)
            data.to_unweighted()
            if hasattr(data, 'rate'):
                data.rate_b = torch.tensor([float(data.rate)], dtype=torch.float32)
            else:
                data.rate_b = None
...
def test(args, model, test_data):
    ...
    with torch.no_grad():
        for i, [data_dir, edge_index_s, pi_edges_signed_tensor, tt_pair_index_tensor,
                node_features_tensor, node_labels_tensor, tt_dis_tensor, rate_b] in enumerate(test_data):
            node_labels_tensor = node_labels_tensor.unsqueeze(1)
            out_emb, out = model(node_features_tensor, edge_index_s, rate_b)
...
def train(args, model, optimizer, train_data, valid_data, logger):
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        for i, [data_dir, edge_index_s, pi_edges_signed_tensor, tt_pair_index_tensor,
                node_features_tensor, node_labels_tensor, tt_dis_tensor, rate_b] in enumerate(train_data):
            node_labels_tensor = node_labels_tensor.unsqueeze(1)
            out_emb, out = model(node_features_tensor, edge_index_s, rate_b)
...
def set_seed(seed):
    ...
    # # for old devices like 3090 and lower, make sure acc
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available():
        ...
        print(f"[INFO] Max GPU memory allocated: {max_mem:.2f} MB")
